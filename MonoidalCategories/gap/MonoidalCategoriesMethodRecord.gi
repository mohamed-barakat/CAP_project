...
TensorProductOnMorphismAndObject := rec(
  filter_list := [ "category", "morphism", "object" ],
  return_type := "morphism",
  input_arguments_names := [ "cat", "alpha", "b" ],
  output_source_getter_string := "TensorProductOnObjects( cat, Source( alpha ), b )",
  output_source_getter_preconditions := [ [ "TensorProductOnObjects", 1 ] ],
  output_range_getter_string := "TensorProductOnObjects( cat, Range( alpha ), b )",
  output_range_getter_preconditions := [ [ "TensorProductOnObjects", 1 ] ],
  with_given_object_position := "both",
  dual_operation := "TensorProductOnMorphismAndObject",
  dual_arguments_reversed := false,
  compatible_with_congruence_of_morphisms := true,
  # Test in MonoidalCategoriesTest
),

TensorProductOnMorphismAndObjectWithGivenTensorProducts := rec(
  ...
  output_range_getter_string := "r",
  output_range_getter_preconditions := [ ],
  dual_operation := "TensorProductOnMorphismAndObjectWithGivenTensorProducts",
  dual_with_given_objects_reversed := true,
  compatible_with_congruence_of_morphisms := true,
),

TensorProductOnObjectAndMorphism := rec(
  filter_list := [ "category", "object", "morphism" ],
  return_type := "morphism",
  input_arguments_names := [ "cat", "a", "beta" ],
  output_source_getter_string := "TensorProductOnObjects( cat, a, Source( beta ) )",
  output_source_getter_preconditions := [ [ "TensorProductOnObjects", 1 ] ],
  output_range_getter_string := "TensorProductOnObjects( cat, a, Range( beta ) )",
  output_range_getter_preconditions := [ [ "TensorProductOnObjects", 1 ] ],
  with_given_object_position := "both",
  dual_operation := "TensorProductOnObjectAndMorphism",
  dual_arguments_reversed := false,
  compatible_with_congruence_of_morphisms := true,
  # Test in MonoidalCategoriesTest
),

TensorProductOnObjectAndMorphismWithGivenTensorProducts := rec(
  ...
  output_range_getter_string := "r",
  output_range_getter_preconditions := [ ],
  dual_operation := "TensorProductOnObjectAndMorphismWithGivenTensorProducts",
  dual_with_given_objects_reversed := true,
  compatible_with_congruence_of_morphisms := true,
),
